...
7. **配置 Web 面板（可选）**：
   ...
   ```env
   # 面板绑定地址（默认 127.0.0.1；Docker/云服务器建议设为 0.0.0.0）
   PANEL_HOST=127.0.0.1

   # 面板端口（避免 3000/8080，默认 8765）
   PANEL_PORT=8765

   # 面板管理 Token（推荐设置）
   # PANEL_ADMIN_TOKEN=your-admin-token
   ```
...
# 编辑 .env（建议设置 PANEL_PORT=8765，PANEL_HOST=0.0.0.0，避免 3000/8080）
...
docker run -d \
  ...
  -p ${PANEL_PORT:-8765}:${PANEL_PORT:-8765} \
