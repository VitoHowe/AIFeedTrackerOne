...
请开始总结："""
    ...
    # 小红书图片分析提示词（基金大 V 今日操作场景）
    XHS_IMAGE_PROMPT = """你是专业的投资研究助理，擅长从图片中提炼基金大 V 的今日操作与思路。请基于图片内容输出结构化总结，要求如下：

## 摘要（必需）
用 80-150 字概括今日操作的核心动作与市场情绪。
...
## 思考与建议（必需）
1. 对当前策略的风险点与假设条件
2. 给普通投资者的操作建议（如仓位控制/分批/止损/回避）

## 风险提示（必需）
强调图中信息可能不完整，不构成投资建议。

输出要求：中文，重点清晰，避免发挥，严格基于图片内容。"""
